...
const generatePDF = (travel: any ) => {
  ...
  // Listar actividades
  const activities = (travel.activities.map((activity: any, index: number) => {
  ...
  const HEADER_FIRST_PAGE = [{
    columns: [
      { image: HEADER.image, width: 615, alignment: "center", margin: [0, 0, 0, 0], absolutePosition: { x: 0, y: 0 } },
    ],
  }, {
  ...
  const docDefinition: any = {
    pageSize: 'LETTER',
    watermark: { text: "Giras Indomito", color: "blue", opacity: 0.1, bold: false },
    content,
    // styles,
    pageMargins: [ 0, 0, 0, 0 ],
  };
